...
def water_ana(sn,\
              Os = np.arange(0,128),Hs = np.arange(128,384),\
              OH_cutoff = 1.25,HB_OO_cutoff = 3.5,HB_HOO_cutoff = 30,fill=0,\
              DF_step = 500,DF_range = (0,6),\
              PTC_range = (-5,1),PTC_OO_cutoff = 3.35,\
              OOO_range = (0,180),OOO_OO_cutoff = 3.35,\
              ba_range = (0,180),\
              search4water=True):
    """
    #set parameters
    ...
    if(search4water):
        #t_Os,t_Hs = np.where(all_dist[Os][:,Hs]<OH_cutoff)
            ...
            #waters = np.vstack((np.unique(Os[t_Os]),Hs[t_Hs].reshape(Os.size,2).T)).T
            #if quantum case H fluctuation large use the closest two as H
        #else:
            #waters = np.vstack((Os,Hs[np.argsort(all_dist[Os][:,Hs],axis=1)[:,:2]].T)).T 
        waters = np.vstack((Os,Hs[np.argsort(all_dist[Os][:,Hs],axis=1)[:,:2]].T)).T 
    else:
        #default water
        waters = np.vstack((Os,Hs.reshape(Os.size,2).T)).T
    nwater = Os.size
    ...
    HB_count = np.bincount(np.sum(water_graph,axis=0)+np.sum(water_graph,axis=1),minlength=15)
    ...
    q = 1 - 3/8*np.average(Sg) #tetrahedral order parameter
    
    D_OOO = DF(DF_step,OOO_range)
    D_OOO.read(OOO_angle)
    
    D_bondangle = DF(DF_step,ba_range)
    D_bondangle.read(sn.get_angles(waters[:,(1,0,2)],mic=True))

    return density,average_HB,HB_accept_count,HB_donate_count,HB_count,dOH,gOO,gOH,gHH,PTC,q,D_OOO,D_bondangle
...
def water_ana2(sn,\
              ...
              OH_cutoff = 1.25,HB_OO_cutoff = 3.5,HB_HOO_cutoff = 30,fill=0,\
              DF_step = 500,DF_range = (0,6),\
              PTC_range = (-5,1),PTC_OO_cutoff = 3.35,PTC_OHO_range = (80,180),\
              OOO_range = (0,180),OOO_OO_cutoff = 3.35,\
              ba_range = (0,180),\
              search4water=True):
    """
    ...
    PTC_OHO = DF2D((DF_step,DF_step),(PTC_range,PTC_OHO_range))
    PTC_OHO.read(all_dist[O1s,waters[O1s,1]]-all_dist[O2s,waters[O1s,1]],sn.get_angles(np.vstack((O1s,waters[O1s,1],O2s)).T,mic=True))
    PTC_OHO.read(all_dist[O1s,waters[O1s,2]]-all_dist[O2s,waters[O1s,2]],sn.get_angles(np.vstack((O1s,waters[O1s,2],O2s)).T,mic=True))
    ...
    #calculate dH_O H-bond length
    HB1O1,HB1O2 = np.where(water_graph1==1)
    HB2O1,HB2O2 = np.where(water_graph2==1)
    ...
    return density,average_HB,HB_accept_count,HB_donate_count,HB_count,dOH,gOO,gOH,gHH,PTC,q,D_OOO,D_bondangle,aHOH,dH_O,PTC_OHO
