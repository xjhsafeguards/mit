            ...
            for(int i=0;i!=f_end;++i){
                ...
                if(i>=f_start and (i-f_start)%f_step == 0){
                        ...
                        if(atom1->check_type("O"))
                            for( const auto& atom2: cel->atoms()){
                                if(atom2->check_type("Cl"))
                                    data[2].push_back(cel->atoms_dis(atom1,atom2));
                                if(atom2->check_type("O")){
                                    data[1].push_back(cel->atoms_dis(atom1,atom2));
                                    if(atom1!=atom2 and cel->atoms_dis(atom1,cel->atoms()[0])<OCl_cutoff and cel->atoms_dis(atom2,cel->atoms()[0])<OCl_cutoff)
                                        data[4].push_back(cel->atoms()[0]->angle(*atom1,*atom2));
                                }
                                if(atom2->check_type("H"))
                                    data[0].push_back(cel->atoms_dis(atom1,atom2));
                            }
                        if(atom1->check_type("H")){
                            for( const auto& atom2: cel->atoms()){
                                if(atom2->check_type("H") and atom1!=atom2 and cel->atoms_dis(atom1,cel->atoms()[0])<HCl_cutoff and cel->atoms_dis(atom2,cel->atoms()[0])<HCl_cutoff)
                                    data[5].push_back(cel->atoms()[0]->angle(*atom1,*atom2));
                                if(atom2->check_type("Cl"))
                                    data[3].push_back(cel->atoms_dis(atom1,atom2));
                            }
                        }
                    }
                    for(int i=0;i!=6;++i){
                    ...
                    for(const auto& mol : cel->mols("H2O")){
                        ...
                        double OCl = cel->atoms_dis(mol->atoms()[0],cel->atoms()[0]);
                        double OH1 = cel->atoms_dis(mol->atoms()[1],mol->atoms()[0]);
                        double OH2 = cel->atoms_dis(mol->atoms()[2],mol->atoms()[0]);
                        if ( OCl < OCl_cutoff){
                            double HCl1 = cel->atoms_dis(mol->atoms()[1],cel->atoms()[0]);
                            double HCl2 = cel->atoms_dis(mol->atoms()[2],cel->atoms()[0]);
                        ...
                        for(const auto& mol2 : cel->mols("H2O")){
                            double OO = cel->atoms_dis(mol->atoms()[0],mol2->atoms()[0]);
                            if( OO < OO_cutoff and OO > 0.001 ){
                                double H1O2= cel->atoms_dis(mol->atoms()[1],mol2->atoms()[0]);
                                double H2O2= cel->atoms_dis(mol->atoms()[2],mol2->atoms()[0]);
