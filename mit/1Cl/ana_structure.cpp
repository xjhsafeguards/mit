...
int main(int argc,char** argv){
    ...
    ofstream ofs[1];
    for(int i=0;i<1;++i){
        ofs[i].open("CN_ptc.dat");
        ofs[i] << setprecision(5);
    }
    ofs[0] << "snapshot min_ptc CNO CNH CNOXX CNOHX CNOHH CNXHH CNXXH" << endl;
            ...
            if(i>=f_start and (i-f_start)%f_step == 0){
                ...
                for(int i=0;i!=f_end;++i){
                    ...
                    if(i>=f_start and (i-f_start)%f_step == 0){
                        ...
                        vector<double> mol_ptc;
                        int count_CNO=0, count_CNH=0, count_CNOXX=0, count_CNOHX=0,count_CNOHH=0,count_CNXHH=0,count_CNXXH=0 ;
                        for(const auto& mol : cel->mols("H2O")){
                            ...
                            bool isH1 = HCl1 < HCl_cutoff;
                            bool isH2 = HCl2 < HCl_cutoff;
                            bool isO = OCl < OCl_cutoff;
                            int numH=0;
                            if(isH1)
                                ++numH;
                            if(isH2)
                            ...
                            count_CNH += numH;
                            mol_ptc.push_back(PTC1);
                            mol_ptc.push_back(PTC2);
                        }
                        /*
                        GTIMER.start("Read Water");
                        water->read(*cel);
                        ...
                        ofs[0] << setw(6) << i << setw(10) << *max_element(mol_ptc.cbegin(),mol_ptc.cend()) << setw(3) << count_CNO << setw(3) << count_CNH << setw(3) << count_CNOXX << setw(3) << count_CNOHX << setw(3) << count_CNOHH << setw(3) << count_CNXHH << setw(3) << count_CNXXH;
                        ofs[0] <<'\n';
