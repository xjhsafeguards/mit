                    ...
                    if(i>=f_start and (i-f_start)%f_step == 0){
                        ...
                        if(count == 6){
                            ...
                            */
                             ...
                             */
                        int count = 63;
                        ofs[0] << count*3+1 << '\n' << "SS: " << i << " PTC: " << *max_element(mol_ptc.cbegin(),mol_ptc.cend());
                        ofs[0] <<'\n';
                        ofs[0] << " Cl 0 0 0" << '\n';
                        int num=0;
                        for(const auto& mol : cel->mols("H2O")){
                            ofs[0] << " O  " << mol->atoms()[0]->cart().shortest_BC(cel->atoms()[0]->cart(),cel->boxp()->diagonal()) << '\n';
                            ofs[0] << " H  " << mol->atoms()[1]->cart().shortest_BC(cel->atoms()[0]->cart(),cel->boxp()->diagonal()) << "   " << mol_ptc[num++] << '\n';
                            ofs[0] << " H  " << mol->atoms()[2]->cart().shortest_BC(cel->atoms()[0]->cart(),cel->boxp()->diagonal()) << "   " << mol_ptc[num++] << '\n';
                        }
                        GTIMER.start("Print");
#ifdef QUANT
