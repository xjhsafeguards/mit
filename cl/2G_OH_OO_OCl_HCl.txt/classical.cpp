...
int main(int argc,char** argv){
    ...
    for(int i=0;i!=21220;++i){
        ...
        cel->read_box(ifs1);
        cel->read_atoms(ifs2);
        vector<vector<double>> data(4); // OH, OO, OCl, HCl, OClO, HClH,
                ...
                if(atom1->check_type("O"))
                    for( const auto& atom2: cel->atoms()){
                        ...
                        if(atom2->check_type("O")){
                            ...
                            //if(atom1->distance(*cel->atoms()[0])<OCl_cutoff and atom2->distance(*cel->atoms()[0])<OCl_cutoff)
                            //data[4].push_back(cel->atoms()[0]->angle(*atom1,*atom2));
                        }
                    }
                if(atom1->check_type("H")){
                    for( const auto& atom2: cel->atoms()){
                        //if(atom2->check_type("H") and atom1->distance(*cel->atoms()[0])<HCl_cutoff and atom2->distance(*cel->atoms()[0])<HCl_cutoff)
    ...
    ofs << "#" << setw(19) << "R" << setw(20) << "OH" << setw(20) << "OO" << setw(20) << "OCl" << setw(20) << "HCl" << endl ;
            ...
            if(atom1->check_type("O"))
                for( const auto& atom2: cel->atoms()){
                    ...
                    if(atom2->check_type("O")){
                        ...
                        if(atom1->distance(*cel->atoms()[0])<OCl_cutoff and atom2->distance(*cel->atoms()[0])<OCl_cutoff)
                            OClO.push_back(cel->atoms()[0]->angle(*atom1,*atom2));
                    }
                }
            if(atom1->check_type("H")){
                for( const auto& atom2: cel->atoms()){
